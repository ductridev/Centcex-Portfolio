...
document.addEventListener("DOMContentLoaded", async () => {
	...
	let sessionUsername = await Storage.getItem("username");
	...
	if(url.searchParams.get("access") === "view") {
		...
		if(!empty(api) || !empty(await Storage.getItem("NoAPI"))) {
	...
	buttonNoAPIMode.addEventListener("click", async () => {
		let json = await Storage.getItem("NoAPI");
		let data;
		if(empty(json)) {
			data = {};
		} else {
		...
		noAPI = new NoAPI(data, "desktop", Storage);
	...
	buttonNoAPIClear.addEventListener("click", async () => {
		...
		document.getElementById("popup-confirm").addEventListener("click", async () => {
			await Storage.removeItem("NoAPI");
			window.location.reload(true);
		});
	});
		...
		if(validLists.includes(list)) {
			...
			for(let j = 0; j < headers.length; j++) {
				...
				header.addEventListener("click", async () => {
					let order = "ascending";
					let currentOrder = await Storage.getItem(list + "SortOrder");
	...
	async function login(username, password) {
		try {
			if(empty(noAPI)) {
						...
						if(validJSON(xhr.responseText)) {
							...
							if("error" in response) {
								...
								if(response.valid) {
									...
									await Storage.setItem("username", response.username);
				...
				xhr.open("POST", api + "accounts/login.php?platform=desktop", true);
	...
	async function logout() {
		try {
			if(empty(noAPI)) {
				...
				xhr.open("GET", api + "accounts/logout.php?platform=desktop&token=" + sessionToken + "&username=" + sessionUsername, true);
				xhr.send();
			}
		} catch(e) {
			reject(e);
	...
	async function checkSession() {
		...
		if(empty(sessionToken) || empty(sessionUsername)) {
			...
			verifySession(sessionToken).then(async response => {
				setTimeout(() => {
					if(divLoadingOverlay.classList.contains("active")) {
						divLoadingOverlay.classList.remove("active");
						divTitlebar.classList.remove("loading");
	...
	async function populateDashboardMarketList(coins) {
		...
		let sortItem = await Storage.getItem("dashboardMarketSortItem");
		let sortOrder = await Storage.getItem("dashboardMarketSortOrder");
	...
	async function changeSortOrder(list, item, order) {
		...
		if(orders.includes(order) && Object.keys(lists).includes(list)) {
			...
			if(valid) {
				await Storage.setItem(list + "SortOrder", order);
				await Storage.setItem(list + "SortItem", item);
					...
					if(Object.keys(coins).length === 0 && settings.transactionsAffectHoldings !== "override" && settings.transactionsAffectHoldings !== "mixed") {
						...
						if(!empty(coins)) {
							parseHoldings(coins).then(async holdings => {
								...
								let sortItem = await Storage.getItem("dashboardHoldingsSortItem");
								let sortOrder = await Storage.getItem("dashboardHoldingsSortOrder");
	...
	function getLocalSettings() {
		...
		return new Promise((resolve, reject) => {
			...
			getServerSettings().then(async (response) => {
				...
				settings.dashboardWatchlist = empty(await Storage.getItem("dashboardWatchlist")) ? "disabled" : await Storage.getItem("dashboardWatchlist");

				settings.sortOrderNotification = empty(await Storage.getItem("sortOrderNotification")) ? "disabled" : await Storage.getItem("sortOrderNotification");
		...
		return new Promise(async (resolve, reject) => {
			try {
				if(!empty(noAPI)) {
					...
					if(empty(await Storage.getItem("NoAPI")) || !empty(await Storage.getItem("token"))) {
						let xhr = new XMLHttpRequest();
						...
						if(!empty(await Storage.getItem("NoAPI")) && validJSON(await Storage.getItem("NoAPI"))) {
							resolve(JSON.parse(await Storage.getItem("NoAPI")).settings);
						} else {
							reject(e);
						}
					}
