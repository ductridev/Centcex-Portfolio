...
export default function Settings({ navigation, route }) {
	...
	async function copyAddress(address) {
		...
		await Clipboard.getStringAsync(addresses[address]);
	...
	async function readData(type) {
		DocumentPicker.getDocumentAsync({ type: "text/csv", copyToCacheDirectory: true }).then(result => {
			RNFS.readFile(result.uri, "ascii").then(data => {
				if (type === "holdings") {
					importHoldings(data);
